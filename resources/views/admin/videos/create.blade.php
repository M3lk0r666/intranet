<x-admin-layout title="Posts" :breadcrumbs="[
    ...
    [
        'name' => 'Multimedia Empresarial - Crear',
    ],
]">
    ...
    <x-wire-card>
        ...
        <div class="p-4">
            ...
            <div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
                <h2 class="text-2xl font-bold text-gray-800 mb-6">Crear Nuevo Video</h2>
                ...
                <form action="{{ route('admin.videos.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-4 mb-4">
                            <label class="inline-flex items-center">
                                <input type="radio" name="video_type" value="url" class="form-radio text-blue-600"
                                    @if (old('video_type', 'url') == 'url') checked @endif onclick="toggleVideoInput()">
                                <span class="ml-2">URL Externa (YouTube, Vimeo, etc.)</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="radio" name="video_type" value="uploaded"
                                    class="form-radio text-blue-600" @if (old('video_type') == 'uploaded') checked @endif
                    ...
                    <!-- URL del Video (visible solo cuando tipo es URL) -->
                    <div id="url-input" style="display: {{ old('video_type', 'url') == 'url' ? 'block' : 'none' }}">
                        <label for="video_url" class="block text-sm font-medium text-gray-700 mb-1">URL del Video
                            *</label>
                        <input type="url" name="video_url" id="video_url" value="{{ old('video_url') }}"
                            placeholder="https://youtube.com/watch?v=..."
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        @error('video_url')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-sm text-gray-500">Soporta YouTube, Vimeo, Dailymotion, etc.</p>
                    </div>

                    <!-- Subir Archivo (visible solo cuando tipo es uploaded) -->
                    <div id="upload-input" style="display: {{ old('video_type') == 'uploaded' ? 'block' : 'none' }}">
                        <label for="video_file" class="block text-sm font-medium text-gray-700 mb-1">Subir Video
                            *</label>
                        <div
                        ...
                        <!-- Vista previa del video (se llenará con JavaScript) -->
                        <div id="video-preview" class="mt-4 hidden">
                            <video id="preview-player" controls class="w-full rounded-lg" style="max-height: 300px;">
